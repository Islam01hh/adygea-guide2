...
import { Sight, Route, Review, UserProfile, CategoryType, Favorite, UserMessage } from "./types";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(collection(db, "favorites"), (snapshot) => {
      const list: Favorite[] = [];
      snapshot.forEach((docSnap) => {
        const item = { ...docSnap.data(), id: docSnap.id } as Favorite;
      ...
      console.warn("Favorites list Firestore subscription skipped:", err);
    });
    return () => unsub();
  }, [currentUser]);

  // Real-time synchronization for admin Feedback Messages
  useEffect(() => {
    ...
    const unsub = onSnapshot(collection(db, "messages"), (snapshot) => {
      ...
      snapshot.forEach((docSnap) => {
        list.push({ ...docSnap.data(), id: docSnap.id } as UserMessage);
      });
      // Sort message cards with dates newest first
      ...
      console.warn("Messages list sync skipped:", err);
    });
    return () => unsub();
  }, [currentUser]);

  // Toggle favorite status in Firestore
  const handleToggleFavorite = async (sightId: string) => {
    if (!currentUser) {
      setAuthError("Пожалуйста, войдите в аккаунт, чтобы добавить место в избранное.");
      setAuthModalOpen(true);
      return;
    }

    const exFav = favorites.find(f => f.sightId === sightId && f.userId === currentUser.uid);

    if (exFav) {
      try {
        await deleteDoc(doc(db, "favorites", exFav.id!));
      } catch (err) {
        console.error("Failed to delete favorite document:", err);
      }
    } else {
      try {
        ...
        console.error("Failed to add favorite document:", err);
      }
    }
  };

  // Delete message inside admin panel
  ...
  // Submit feedback suggestion to collection "messages" in Firestore
  const handleSubmitFeedback = async (name: string, email: string, message: string): Promise<boolean> => {
    try {
      ...
      console.error("Failed to post custom message suggestion:", err);
      return false;
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans" id="web_doc_root">
      ...
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-100 shadow-sm transition">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="font-display font-black text-teal-900 text-sm md:text-lg tracking-tight uppercase leading-none">
                Адыгея Гид
              </h1>
          ...
          <div className="hidden md:block">
            <span className="text-xs bg-slate-50 border border-slate-150 font-bold text-slate-500 px-4 py-1.5 rounded-2xl uppercase tracking-wider font-mono shadow-sm">
              🏔️ Официальный Портал Республики Адыгея
            </span>
          </div>
      ...
      {/* DYNAMIC NAVIGATION SUB-BAR (Horizontal Desktop & Mobile responsive sticky scroll bar) */}
      <div className="bg-white border-b border-slate-100 shadow-sm sticky top-16 z-30 overflow-x-auto scrollbar-none" id="horizontal_dynamic_sub_nav_row">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2.5 flex items-center gap-2 whitespace-nowrap">
          {[
            { id: "catalog", label: "🏔️ Достопримечательности" },
            { id: "routes", label: `🧭 Маршруты (${routes.length})` },
            { id: "map", label: "🗺️ Интерактивная карта" },
            ...
            <button
              ...
              onClick={() => {
                ...
                setSelectedRoute(null);
                // Reset category filters when toggling tabs
                setSelectedCategory("all");
              }}
              className={`px-3.5 py-1.5 rounded-xl text-xs md:text-sm font-semibold transition flex items-center gap-1.5 ${
                activeTab === tab.id
                  ? "bg-teal-650 text-white shadow-sm font-black"
                  : "bg-slate-50 text-slate-600 hover:bg-slate-100 hover:text-slate-850 border border-slate-200/40"
              }`}
              id={`sub_nav_tab_trigger_${tab.id}`}
            >
              {tab.label}
            </button>
          ))}
          {currentUser && (
            <button
              ...
              className={`px-3.5 py-1.5 rounded-xl text-xs md:text-sm font-bold transition flex items-center gap-1.5 ${
                activeTab === "profile"
                  ? "bg-red-500 text-white shadow-sm"
                  : "bg-red-50 text-red-650 hover:bg-red-100 border border-red-100/50 animate-pulse"
              }`}
              id="sub_nav_tab_trigger_profile"
            >
              ❤️ Кабинет ({favorites.length})
            </button>
          )}
          {isAdminUser && (
            <button
      ...
      <main className="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-8 w-full">
        ...
        {activeTab === "contacts" && (
          <div className="mb-6 animate-fade-in">
            <Contacts onSubmitFeedback={handleSubmitFeedback} />
          </div>
        )}
        ...
        {(activeTab === "catalog" || activeTab === "routes") && (
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="h-44 overflow-hidden relative">
                          ...
                          {/* Heart Icon Button overlay on sight card image */}
                          <button
                            onClick={(e) => {
                              // CRITICAL: Stop propagation to prevent card detail trigger on background catalog click
                              e.stopPropagation();
                              handleToggleFavorite(sight.id);
                            }}
                            className={`absolute top-2.5 right-2.5 p-2 rounded-full backdrop-blur-md transition shadow z-10 ${
                              favorites.some(f => f.sightId === sight.id)
                                ? "bg-red-500 text-white hover:bg-red-600"
                                : "bg-black/40 text-white hover:bg-black/60 hover:scale-105"
                            }`}
                            id={`heart_toggle_list_${sight.id}`}
                            title={favorites.some(f => f.sightId === sight.id) ? "Убрать из избранного" : "Добавить в избранное"}
                          >
                            <Heart className={`w-3.5 h-3.5 ${favorites.some(f => f.sightId === sight.id) ? "fill-current" : ""}`} />
                          </button>

                          <div className="absolute bottom-2.5 left-2.5 bg-teal-600 text-white px-2.5 py-0.5 rounded-lg text-[9px] font-bold tracking-wide uppercase">
                            {sight.category}
                          </div>
                        </div>
      ...
      <footer className="bg-slate-900 text-slate-400 text-xs py-8 border-t border-slate-800 shrink-0 mt-12 text-center">
        <div className="max-w-7xl mx-auto px-4 space-y-3">
          <p className="font-display font-bold text-sm text-slate-100 tracking-wide uppercase">
            🌲 Адыгея Гид — Интерактивный путеводитель по Республике Адыгея
          </p>
          <p className="text-slate-500/90 leading-relaxed max-w-xl mx-auto text-[11px]">
            Разработано в качестве инновационного интеллектуального туристско-информационного комплекса Кавказа. Интегрировано с Firebase Auth, Cloud Firestore и базами данных ИИ.
          </p>
