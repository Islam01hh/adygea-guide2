...
import { Sight, Route, Review, CategoryType, DifficultyType, UserMessage } from "../types";
import { Plus, Trash2, Database, UploadCloud, MapPin, Compass, AlertCircle, Sparkles, Check, Mail, User } from "lucide-react";
...
export default function AdminPanel({
  ...
  // Tabs
  const [activeTab, setActiveTab] = useState<"sights" | "routes" | "reviews" | "seed" | "messages">("sights");
    ...
    <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-xl" id="admin_panel_container">
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {[
          ...
          { id: "messages", label: `📧 Сообщения (${messages.length})` },
      ...
      <div className="space-y-4">
        ...
        {/* --- TABS 5: USER MESSAGES / SUGGESTIONS --- */}
        {activeTab === "messages" && (
          <div className="space-y-4 text-left">
            <div className="border-b border-slate-100 pb-3">
              ...
              <p className="text-slate-500 text-xs">
                Обращения, отзывы и предложения о сотрудничестве, отправленные через контактные формы.
              </p>
            </div>

            {messages.length === 0 ? (
              <div className="p-8 text-center text-slate-400 text-xs bg-slate-50 border border-slate-100 rounded-2xl">
                Сообщений пока нет. Пользователи увидят форму в разделе "Контакты".
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {messages.map((msg, idx) => (
                  <div key={msg.id || idx} className="bg-slate-50 p-4 rounded-2xl border border-slate-100 flex flex-col justify-between space-y-3 shadow-sm hover:shadow transition">
                    <div className="space-y-1.5">
                      ...
                      <p className="text-slate-600 text-xs bg-white p-2.5 rounded-xl border border-slate-150 leading-relaxed font-light min-h-[60px] whitespace-pre-wrap">
                        {msg.message}
                      </p>
                    </div>

                    <div className="pt-2 border-t border-slate-150 flex items-center justify-between text-[11px] text-slate-400">
                      <span>
                        {msg.createdAt && typeof msg.createdAt === 'string' ? msg.createdAt.split('T')[0] : 'Получено только что'}
                      </span>
                      <a
                        href={`mailto:${msg.email}?subject=Адыгея Гид: Ответ на Ваше обращение&body=Здравствуйте, ${msg.name}!%0D%0DСпасибо за Ваше предложение по улучшению путеводителя Адыгеи: "${msg.message.substring(0, 60)}...".%0D%0DМы обязательно учтем Ваш отзыв.%0D%0DС уважением,%0DАдыгея Гид`}
                        className="px-3 py-1.5 bg-teal-600 hover:bg-teal-700 text-white font-bold text-xs rounded-lg transition inline-flex items-center gap-1 shadow-sm hover:shadow"
                        id={`reply_msg_btn_${msg.id || idx}`}
                      >
